...
const CharacterSheet = ({character = "default"}) => {
  ...
  const handleDrag = ({perX, perY, w, h, id, page}) => {
    if (!perX){
      setDragData({})
    }
  ...
  const handleModules = (newModules, page) => {
  ...
  const removeModule = (module, page) => {
    ...
    let updatedModules = [...modules]
    updatedModules[page - 1] = updatedModules[page - 1].filter(mod => {
  ...
  const renderPages = () => {
    return modules.map((pageModules, index) => {
      const page = index + 1
      return (
        <Page 
          dragData={dragData}
